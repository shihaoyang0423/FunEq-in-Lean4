...
(f : ℝ → ℝ)
(h : ∀ x : ℝ, f x = x + 1) :
∀ x y : ℝ, f ((x - y)^2) = (f x)^2 - 2 * x * f y + y^2 := by
  sorry
...
theorem intermediate_funeq_3
  ...
  (h₀ : ∀ x y, f (x + y) = f x + f y)
  (h₁ : ∀ x, x ≠ 0 → f (1/x) = f x/x^2) :
  ∀ x, (x ≠ 0 ∧ x ≠ 1) → f (x^2 - x) = x^2 * f (x - 1) - (x - 1)^2 * f x := by
  sorry
...
theorem intermediate_funeq_7
  ...
  ∀ x z : ℝ, f z * f (x * z)^2 - f x * (1 + f z^2) * f (x * z) + f z * f x^2 = 0 := by
  sorry
...
theorem imo_2022_a6
  ...
  ∀ q : ℚ, (∃ z : ℝ, f z = q * z) ↔ (∃ n : ℤ, n ≠ 0 ∧ q = (n + 1) / n) := by
  sorry
...
theorem imo_2021_a8
  ...
  ∃ (α : ℝ) (β : ℝ), (α = -1 ∨ α = 0 ∨ α = 1) ∧ ((∀ x : ℝ, f x = α * x + β) ∨ (∀ x : ℝ, f x = α * x^3 + β)) := by
  sorry
...
theorem imo_2018_a5
  ...
  ∃ C1 C2 : ℝ, ∀ x : ℝ, x > 0 → f x = C1 * x + C2 / x := by
  sorry

theorem imo_2017_a6
  (f : ℝ → ℝ)
  (h : ∀ x y : ℝ, f (f x * f y) + f (x + y) = f (x * y)) :
  (∀ x : ℝ, f x = 0) ∨ (∀ x : ℝ, f x = x - 1) ∨ (∀ x : ℝ, f x = 1 - x) := by
  sorry
...
theorem imo_2014_a6
  ...
  (∀ n : ℤ, (n > 0 → f n = n + 1) ∧ (n = 0 → f n = 0) ∧ (n < 0 → f n = -n + 1)) := by
  sorry
...
theorem imo_2012_a1
  ...
  (∀ k : ℤ, k ≠ 0 → ((∀ x : ℤ, f x = k * x^2) ∨
             ...
             (∀ x : ℤ, (x % 4 = 0 → f x = 0) ∧ (x % 4 = 1 → f x = k) ∧ (x % 4 = 2 → f x = 4 * k)))) := by
  sorry
...
theorem imo_2011_a3
  ...
  (∀ x : ℝ, f x = 0 ∧ g x = 0) ∨ (∃ C : ℝ, ∀ x : ℝ, f x = x^2 + C ∧ g x = x) := by
  sorry
...
theorem imo_2008_a1
  ...
  (∀ x : ℝ, (0 < x) → f x = x) ∨ (∀ x : ℝ, (0 < x) → f x = 1/x) := by
  sorry
...
theorem imo_2005_a4
  ...
  (∀ x : ℝ, f x = 2 * x - 1) ∨ (∀ x : ℝ, f x = -x - 1) ∨ (∀ x : ℝ, f x = x^2 - 1) := by
  sorry
...
theorem imo_2004_a6
  ...
  (∀ x : ℝ, f x = x) ∨ (∀ x : ℝ, f x = 0) ∨ (∀ x : ℝ, (x ∉ X → f x = 1) ∧ (x ∈ X → f x = -1)) := by
  sorry
...
theorem imo_2002_a4
  ...
  (∀ x : ℝ, f x = 0) ∨ (∀ x : ℝ, f x = 1/2) ∨ (∀ x : ℝ, f x = x^2) := by
  sorry
